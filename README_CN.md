...
- Python 3.11+
- 已安装并使用过 [Claude Code](https://claude.ai/claude-code)
...
claude-buddy config set output-dir ~/reports   # 设置后报告自动保存到该目录
...
```bash
# Anthropic（默认）
claude-buddy config set provider anthropic
claude-buddy config set api-key sk-ant-...
claude-buddy config set model claude-sonnet-4-6

# OpenAI
claude-buddy config set provider openai
claude-buddy config set api-key sk-...
claude-buddy config set model gpt-4o

# Ollama（本地，无需 API Key）
claude-buddy config set provider ollama
claude-buddy config set model llama3.2

# DeepSeek / Moonshot 等兼容 OpenAI 接口的服务
claude-buddy config set provider deepseek
claude-buddy config set api-key sk-...
claude-buddy config set model deepseek-chat
claude-buddy config set base-url https://api.deepseek.com
```
...
claude-buddy report -i 3                         # 用序号指定（来自 list）
...
claude-buddy report -i 3 --no-render             # 输出纯文本 Markdown
```

**API Key 优先级**：命令行 `-k` > 环境变量（`ANTHROPIC_API_KEY` / `OPENAI_API_KEY`）> 配置文件
...
- `thinking` — Claude 的思考过程（任务规划、方案权衡）
- `tool_use` — 工具调用（Bash、Edit、Read、Write、Grep 等）
- `tool_result` — 工具返回结果
- `text` — Claude 对用户的回复

Claude Buddy 解析这些记录，提取有效信息压缩为结构化摘要，交给 LLM 生成可读的教学报告。
